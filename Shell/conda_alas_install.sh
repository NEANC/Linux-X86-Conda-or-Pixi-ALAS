...
_select_fastest_conda_mirror() {
    ...
    for entry in "${_MIRROR_BASES_SORTED[@]}"; do
        ...
        cf_code=$(timeout 10 curl -o /dev/null -s --connect-timeout 3 --max-time 5 -w '%{http_code}' "${candidate}/cloud/conda-forge/" 2>/dev/null)
        _log_message "INFO" "  验证 ${candidate}/cloud/conda-forge/ → HTTP ${cf_code:-超时}"

        pkgs_code=$(timeout 10 curl -o /dev/null -s --connect-timeout 3 --max-time 5 -w '%{http_code}' "${candidate}/pkgs/main/" 2>/dev/null)
