...
"@
    Write-Host "  `u{1F5A5}`u{FE0F}  Windows 中基于 Pixi 的 ALAS 部署脚本"
    Write-Host "  ─────────────────────────────────────────────────"
...
function Find-PixiExe {
    $candidates = @()

    if ($env:PIXI_HOME) {
        $candidates += Join-Path $env:PIXI_HOME "bin\pixi.exe"
    }

    $candidates += @(
        "$env:USERPROFILE\.pixi\bin\pixi.exe",
        "$env:USERPROFILE\.pixi\bin\pixi",
        "$env:LOCALAPPDATA\pixi\bin\pixi.exe",
        "$env:LOCALAPPDATA\pixi\bin\pixi",
        "$env:APPDATA\pixi\bin\pixi.exe",
        "$env:APPDATA\pixi\bin\pixi",
        "${env:ProgramFiles}\pixi\bin\pixi.exe",
        "${env:ProgramFiles(x86)}\pixi\bin\pixi.exe"
    )
    ...
    $cmdPixi = Get-Command pixi -ErrorAction SilentlyContinue
    if ($cmdPixi) {
        return $cmdPixi.Source
    }

    try {
        $whereResult = where.exe pixi 2>$null
...
function Install-Pixi {
    Start-Step "正在检查 Pixi 包管理器..."

    $foundPixi = Find-PixiExe
    if ($foundPixi) {
...
mxnet = { url = "https://github.com/swordfeng/mxnet-build/releases/download/rev1/mxnet-1.9.1-cp38-cp38-win_amd64.whl" }
[dependencies]
python = "==3.8.10"
av = ">=10.0.0,<11"
numpy = "==1.24.4"
...
set "PIXI_BIN=$PixiBin"
set "PYTHONUTF8=1"
cd /d "%ALAS_DIR%"
start "" http://127.0.0.1:22267
"%PIXI_BIN%" run --manifest-path "%ALAS_DIR%\pixi.toml" start
"@
...
function Write-Completion {
    ...
    if ($SkipService) {
        ...
        Write-Host "  `u{1F4A1}  或:  cd $AlasDir && pixi run start" -ForegroundColor Cyan
    }
    Write-Host ""
}
